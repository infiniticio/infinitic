...
class PulsarInfiniticAdmin(val pulsarAdmin: PulsarAdmin, val pulsar: Pulsar) : Closeable {
  ...
  companion object {
    ...
    @JvmStatic
    fun from(pulsarAdmin: PulsarAdmin, adminConfig: AdminConfig) =
        PulsarInfiniticAdmin(pulsarAdmin, adminConfig.pulsar)
  ...
  val topics: Set<String> by lazy { pulsarTopics.getPartitionedTopicList(fullNamespace).toSet() }
  ...
  fun createTenant(): Boolean =
      try {
        ...
        logger.warn {
          "Not authorized - make sure by yourself that tenant '${pulsar.tenant}' exists - with the right info"
        }
        false
      } catch (e: PulsarAdminException.NotAllowedException) {
        logger.warn {
          "Not allowed - make sure by yourself that tenant '${pulsar.tenant}' exists - with the right info"
        }
        false
      }
  ...
  fun createNamespace(): Boolean =
      try {
        ...
        logger.warn {
          "Not authorized - make sure by yourself that namespace '$fullNamespace' exists - with the right policies"
        }
        false
      } catch (e: PulsarAdminException.NotAllowedException) {
        logger.warn {
          "Not allowed - make sure by yourself that namespace '$fullNamespace' exists - with the right policies"
        }
        false
      }
   ...
   */
  fun createTopic(topic: String, isPartitioned: Boolean): Boolean =
      try {
        ...
        logger.warn {
          "Not allowed - make sure by yourself that topic '$topic' exists - with the right policies"
        }
        false
      } catch (e: PulsarAdminException.NotAuthorizedException) {
        logger.warn {
          "Not authorized - make sure by yourself that topic '$topic' exists - with the right policies"
        }
        false
      }
  ...
  fun updateNamespacePolicies() {
    with(pulsarNamespaces) {
      ...
      try {
        logger.info {
          "Updating policy SchemaValidationEnforced to ${policies.schema_validation_enforced}"
        }
      ...
      try {
        logger.info {
          "Updating policy IsAllowAutoUpdateSchema to ${policies.is_allow_auto_update_schema}"
        }
      ...
      try {
        logger.info {
          "Updating policy SchemaCompatibilityStrategy to ${policies.schema_compatibility_strategy}"
        }
      ...
      try {
        logger.info {
          "Updating policy DelayedDeliveryMessages to ${policies.delayed_delivery_policies}"
        }
        setDelayedDeliveryMessages(fullNamespace, policies.delayed_delivery_policies)
      } catch (e: PulsarAdminException) {
        logger.warn {
          "Failing to update namespace's delayed_delivery_policies policy: ${e.message}"
        }
      }
    }
  }
  ...
  fun printTopicStats() {
    ...
    val line =
        "+--------------------------------------------+-------------+------------+------------+%n"
    val title =
        "| Subscription                               | NbConsumers | MsgBacklog | MsgRateOut |%n"
