...
interface WorkerRegister {
  ...
  fun registerService(name: String, factory: ServiceFactory, concurrency: Int) =
      registerService(name, factory, concurrency, null, null, DEFAULT_TASK_TAG)

  fun registerService(name: String, factory: ServiceFactory) =
  ...
  fun registerWorkflow(
      ...
      registerWorkflow(
          name, classes, concurrency, timeout, retry, checkMode, engine, DEFAULT_WORKFLOW_TAG)
  ...
  fun registerWorkflow(
      ...
      checkMode: WorkflowCheckMode?,
      engine: WorkflowEngine?
  ) = registerWorkflow(name, listOf(`class`), concurrency, timeout, retry, checkMode, engine)
  ...
  fun registerWorkflow(
      ...
      retry: WithRetry?,
      checkMode: WorkflowCheckMode?
  ) = registerWorkflow(name, listOf(`class`), concurrency, timeout, retry, checkMode)
  ...
  fun registerWorkflow(
      ...
      timeout: WithTimeout?,
      retry: WithRetry?
  ) = registerWorkflow(name, listOf(`class`), concurrency, timeout, retry)
  ...
  fun registerWorkflow(
      ...
      concurrency: Int,
      timeout: WithTimeout?
  ) = registerWorkflow(name, listOf(`class`), concurrency, timeout)

  fun registerWorkflow(name: String, classes: WorkflowClassList, concurrency: Int) =
      registerWorkflow(
          name,
          classes,
          concurrency,
  ...
  fun registerWorkflow(name: String, `class`: Class<out Workflow>, concurrency: Int) =
      registerWorkflow(name, listOf(`class`), concurrency)

  fun registerWorkflow(name: String, classes: WorkflowClassList) =
      registerWorkflow(
          name,
          classes,
          DEFAULT_CONCURRENCY,
  ...
  fun registerWorkflow(name: String, `class`: Class<out Workflow>) =
      registerWorkflow(name, listOf(`class`))
}
