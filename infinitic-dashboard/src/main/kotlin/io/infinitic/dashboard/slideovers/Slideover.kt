...
class Slideover<T>(
    ...
    private val renderState: ElementCreator<Element>.(state: KVar<T>) -> Unit = { div().classes("h-full border-2 border-dashed border-gray-200") }
) {
    ...
    fun render(creator: ElementCreator<Element>): Unit = with(creator) {
        div().classes("fixed overflow-hidden")
