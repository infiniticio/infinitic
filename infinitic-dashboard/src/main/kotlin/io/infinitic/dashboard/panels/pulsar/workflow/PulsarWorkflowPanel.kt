...
class PulsarWorkflowPanel private constructor(private val workflowName: String) : Panel() {
    companion object {
        private val instances: ConcurrentHashMap<String, PulsarWorkflowPanel> = ConcurrentHashMap()

        fun from(workflowName: String) = instances.computeIfAbsent(workflowName) { PulsarWorkflowPanel(workflowName) }
    }

    override val menu = PulsarMenu

    override val route = "/pulsar/w/$workflowName"

    private val workflowState = KVar(PulsarWorkflowState(workflowName))
    private val workflowTaskState = KVar(PulsarWorkflowTaskState(workflowName))
