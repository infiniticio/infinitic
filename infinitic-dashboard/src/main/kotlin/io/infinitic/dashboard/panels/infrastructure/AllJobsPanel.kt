...
object AllJobsPanel : Panel() {
  ...
  override fun render(creator: ElementCreator<Element>) =
      with(creator) {
        // PAGE HEADER
                ...
                div().classes("bg-white overflow-hidden sm:rounded-lg sm:shadow").new {
                  div().classes("flex flex-col").new {
                    div().classes("-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8").new {
                      div()
                          .classes("py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8")
                          .new {
                            div()
                                .classes(
  ...
  private fun ElementCreator<Element>.displayExecutorError(name: String, type: JobType) {
    ...
    row.classes("bg-white cursor-pointer hover:bg-gray-50").new {
      td().classes("px-6 py-4 text-sm font-medium text-gray-900").text(name)
      td().classes("px-6 py-4 text-sm text-gray-500").text("error!")
      td().classes("px-6 py-4 text-sm text-gray-500").text("error!")
      td().classes("px-6 py-4 text-sm text-gray-500").text("error!")
    }
    row.on.click {
...
internal fun lastUpdated(instant: Instant) =
    "Last updated at ${SimpleDateFormat("HH:mm:ss").format(Date.from(instant))}"
