...
abstract class AllJobsState(
    ...
    companion object {
        ...
        fun lastUpdatedAt(names: JobNames, stats: JobStats): Instant {
            ...
            return when (statsLastUpdated) {
                ...
                else -> when (statsLastUpdated.isAfter(namesLastUpdated)) {
    ...
    abstract fun create(
        names: JobNames = this.names,
        stats: JobStats = this.stats
    ): AllJobsState
    ...
    while (isActive) {
        with(kvar) {
            ...
            try {
                ...
                value = value.create(names = Failed(e), stats = mapOf(),) as T
            }
