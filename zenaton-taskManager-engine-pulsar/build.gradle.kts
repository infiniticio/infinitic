...
application {
    mainClassName = "com.zenaton.taskManager.pulsar.utils.GenerateSchemaFilesKt"
}
...
tasks.register("update") {
    ...
    doLast {
        ...
        setZenatonFunction(
            name = "infinitic-tasks-monitoring-global",
            className = "MonitoringGlobalPulsarFunction",
            classNamespace = "com.zenaton.taskManager.pulsar.functions",
            topicsIn = setOf(Topic.MONITORING_GLOBAL.get()),
            action = "update"
        )
        setZenatonFunction(
...
val jar = "zenaton-taskManager-engine-pulsar-1.0.0-SNAPSHOT-all.jar"
...
fun createSchemaFiles() {
    ...
    val cmd = "java -cp ./build/libs/$jar com.zenaton.taskManager.pulsar.utils.GenerateSchemaFilesKt"
    return exec(cmd)
}
...
fun uploadSchemaToTopic(
    ...
    val cmd = "$pulsarAdmin schemas upload \"persistent://$tenant/$namespace/$topic\"" +
        " --filename \"/zenaton/taskManager/schemas/$name.schema\" "
    return exec(cmd)
}
...
fun setZenatonFunction(
    name: String,
    className: String,
    classNamespace: String = "com.zenaton.taskManager.pulsar.functions",
    topicsIn: Set<String>,
    action: String,
    ...
    var cmd = "$pulsarAdmin functions $action --jar /zenaton/taskManager/libs/$jar" +
