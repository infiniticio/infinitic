...
interface TagStateStorage {
    ...
    suspend fun getState(tag: Tag): TagState? {
        val tagState = getStateFn(tag)
        logger.debug("workflowTag {} - getState {}", tag, tagState)

        return tagState
    }

    suspend fun puState(tag: Tag, state: TagState) {
        putStateFn(tag, state)
        logger.debug("workflowTag {} - updateState {}", tag, state)
    }

    suspend fun delState(tag: Tag) {
        delStateFn(tag)
        logger.debug("workflowTag {} - deleteState", tag)
    }
}
