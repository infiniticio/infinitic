...
data class WorkflowExecutorEnvelope(
    ...
    val type: WorkflowExecutorMessageType,
    val runWorkflow: RunWorkflow? = null
) {
    init {
        val noNull = listOfNotNull(runWorkflow)
    ...
    companion object {
        fun from(msg: WorkflowExecutorMessage) = when (msg) {
            is RunWorkflow -> WorkflowExecutorEnvelope(
                msg.workflowName,
                WorkflowExecutorMessageType.RUN_WORKFLOW,
                runWorkflow = msg
            )
        }
