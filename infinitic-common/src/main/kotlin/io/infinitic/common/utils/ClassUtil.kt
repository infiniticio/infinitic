 ...
 */
fun Class<*>.getFullMethodName(method: Method) = "${this.findSimpleName()}::${method.findName()}"
...
fun Method.findName(): String = methodNameCache.getOrPut(this) {
...
private fun Class<*>.findSimpleName(): String = classSimpleNameCache.getOrPut(this) {
  // Use @Name annotation if any
  findAnnotation(Name::class.java)?.name
  // else use class name
    ?: simpleName
...
fun Class<*>.findName(): String = classNameCache.getOrPut(this) {
  // Use @Name annotation if any
  findAnnotation(Name::class.java)?.name
  // else use class name
    ?: name
...
fun Method.getCheckMode(): WorkflowCheckMode? = workflowCheckModeCache.getOrPut(this) {
...
fun Method.getWithRetry(): Result<WithRetry?> = methodWithRetryCache.getOrPut(this) {
...
fun Method.getWithTimeout(): Result<WithTimeout?> = methodWithTimeoutCache.getOrPut(this) {
...
fun Method.isDelegated(): Boolean = methodIsDelegatedCache.getOrPut(this) {
...
fun <T : Any> Method.getMillisDuration(klass: Class<T>): Result<MillisDuration?> {
  val millis: Long? = (this.getTimeoutInMillis() ?: klass.getTimeoutInMillis())
      ?.getOrElse { return Result.failure(it) }
...
private fun Method.getTimeoutInMillis(): Result<Long?>? = methodTimeoutCache.getOrPut(this) {
...
internal fun <T : Any> Class<T>.getTimeoutInMillis(): Result<Long?>? =
    classTimeoutCache.getOrPut(this) {
...
internal fun Method.getJsonViewClass(): Class<*>? = methodJsonViewClassCache.getOrPut(this) {
