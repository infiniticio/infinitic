...
class MonitoringPerNameEngineWorker(storage: KeyValueStorage) : CoroutineScope {

    override val coroutineContext: CoroutineContext
        get() = Dispatchers.IO + Job()

    private val monitoringPerNameStateStorage = MonitoringPerNameStateKeyValueStorage(storage)
    ...
    fun start(pulsarClient: PulsarClient) {
        ...
        val workflowEngine = MonitoringPerNameEngine(
            monitoringPerNameStateStorage,
            transport.sendToMonitoringGlobalEngine
        )

        try {
            val workflowEngineConsumer = pulsarClient.newMonitoringPerNameEngineConsumer()
