...
fun CoroutineScope.startTaskEngine(
    taskStateStorage: TaskStateStorage,
    taskClientChannel: ReceiveChannel<TaskEngineMessage>,
    taskResultChannel: Channel<TaskEngineMessage>,
    executorChannel: SendChannel<TaskExecutorMessage>,
    monitoringPerNameChannel: SendChannel<MonitoringPerNameEngineMessage>,
    ...
    // TODO store delayed messages
    val sendToTaskEngine: SendToTaskEngine = { msg: TaskEngineMessage, after: Float ->
    ...
    val sendToMonitoringPerName: SendToMonitoringPerName = { monitoringPerNameChannel.send(it) }

    val sendToExecutors: SendToExecutors = { executorChannel.send(it) }
    ...
    while (true) {
        select<Unit> {
            taskResultChannel.onReceive { taskEngine.handle(it) }
            taskClientChannel.onReceive { taskEngine.handle(it) }
        }
    }
}
