...
class MonitoringGlobalEngineWorker(storage: KeyValueStorage) : CoroutineScope {

    override val coroutineContext: CoroutineContext
        get() = Dispatchers.IO + Job()

    private val monitoringGlobalStateStorage = MonitoringGlobalStateKeyValueStorage(storage)
    lateinit var shutdownHook: (() -> Unit)
    ...
    fun stop() {
        cancel()
        if (this::shutdownHook.isInitialized) { shutdownHook() }
    }

    fun start(pulsarClient: PulsarClient) {
        shutdownHook = { pulsarClient.close() }

        val workflowEngine = MonitoringGlobalEngine(
            monitoringGlobalStateStorage
        )

        try {
            val workflowEngineConsumer = pulsarClient.newMonitoringGlobalEngineConsumer()
