...
class PulsarInfiniticProducer(
  ...
  private suspend fun sendEnvelope(
    ...
    producerName: String,
    key: String? = null
  ): Unit = sendEnvelopeAsync(envelope, after, topicName, producerName, key).await()

  private fun sendEnvelopeAsync(
    envelope: Envelope<out Message>,
    after: MillisDuration,
    topicName: String,
    producerName: String,
    ...
    val producer = client
        .getProducer(topicName, envelope::class, producerName, null, pulsarProducerConfig, key)
        .getOrElse { return CompletableFuture.failedFuture(it) }

    logger.trace { "Sending${if (after > 0) " after $after ms" else ""} to topic '$topicName' with key '$key': '$envelope'" }
